...
const AppTextInput: React.FC<PropsType> = ({
  onTextChange,
  width,
  placeholder,
  error,
  ...
  const [borderColor, setBorderColor] = useState(Colors.PROGRESS_GREY);
  ...
  useEffect(() => {
    ...
    if (!focused) {
      ...
      setBorderColor(Colors.PROGRESS_GREY);
    }
  }, [focused]);
  ...
  return (
    <View
      style={{
        ...styles.container,
        backgroundColor: focused ? Colors.DEFAULT_WHITE : Colors.DEFAULT_WHITE,
        borderWidth: 1,
        borderRadius: 8,
        width: width || '100%',
        paddingHorizontal: 6,
      ...
      <TextInput
        ...
        placeholderTextColor={Colors.PROGRESS_GREY}
      />
    </View>
  );
};
